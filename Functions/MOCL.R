...
MOCL<-function(Xf,k,max.iter=500,tr_ratio=0.8,num.knotsf=6,max.lambda=100,r_a=0.01,seed=100,delta=0.01){
  ...
  for(i in 1:iter){
    ...
    # Stopping rule
    if(i>1){
      if(sum(cluster_matrix[,i-1]!=cluster_matrix[,i])<(max(1,delta*n)+1)){
      break
      }
    }
  ...
  if(stop>=max.iter){
    ...
    return(list(cluster=cluster, cluster_matrix=cluster_matrix,cluster_matrix_score=cluster_matrix_score,m_lambda=m_lambda,
                iteration=(stop+1),f_x=function_mat,y_tr_mean=y_mean))
  }
}
